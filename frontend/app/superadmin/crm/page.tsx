...
export default function CRMPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[32px] font-bold text-[#F8FAFC]">CRM Kanban</h1>
        <p className="text-[#94A3B8] mt-1">Gerencie o funil de vendas</p>
      </div>
        ...
        {colunas.map((col, colIdx) => (
          <div key={colIdx} className="min-w-[280px]">
            <div className={`border-t-4 ${col.cor} bg-[#1E293B] rounded-lg p-4 border border-[#1E293B]`}>
              <h3 className="font-semibold text-[#F8FAFC] mb-3">{col.nome}</h3>
              <div className="space-y-3">
                {col.cards.map((card, cardIdx) => (
                  <Card key={cardIdx} className="p-4 bg-[#0F172A] border-[#1E293B] rounded-lg hover:border-[#8B5CF6]/50 transition-colors cursor-move">
                    <p className="font-medium text-[#F8FAFC] text-sm">{card.empresa}</p>
                    <p className="text-xs text-[#94A3B8] mt-1">{card.contato}</p>
