  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-[32px] font-bold text-[#F8FAFC]">Monitoramento</h1>
          <p className="text-[#94A3B8] mt-1">Status em tempo real dos recursos e containers</p>
        </div>
        <span className="text-sm text-[#94A3B8]">Atualizado: {new Date().toLocaleTimeString('pt-BR')}</span>
      </div>
        ...
        {[
          ...
          <Card key={idx} className="p-6 bg-[#1E293B] border-[#1E293B] rounded-xl">
            ...
            <div className="flex items-end gap-2 mt-2">
              <span className="text-[32px] font-bold text-[#F8FAFC]">{item.value}</span>
              <span className="text-sm text-[#94A3B8] mb-1">{item.unit}</span>
            </div>
            <div className="h-2 bg-[#0F172A] rounded-full mt-4 overflow-hidden">
      ...
      <Card className="p-6 bg-[#1E293B] border-[#1E293B] rounded-xl">
        <h2 className="text-[20px] font-semibold text-[#F8FAFC] mb-4">Containers Docker</h2>
            ...
            <tbody className="divide-y divide-[#1E293B]">
              {[
                ...
                <tr key={idx} className="hover:bg-[#0F172A]/50">
                  <td className="py-4 font-mono text-sm text-[#F8FAFC]">{container.name}</td>
                  <td className="py-4"><span className="text-[#10B981]">🟢 {container.status}</span></td>
                  <td className="py-4 text-[#94A3B8]">{container.cpu}</td>
                  <td className="py-4 text-[#94A3B8]">{container.mem}</td>
                  <td className="py-4 text-[#94A3B8]">{container.uptime}</td>
                </tr>
              ))}
            </tbody>
          </table>
